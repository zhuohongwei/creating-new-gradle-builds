...
NOTE: Shell commands will be shown for Unix-based systems, but we will identify the changes needed for the Windows command prompt as and when necessary.
...
[TIP]
====
You can also run the task using the shorthand `gradle cS`. Read more about task name abbreviation in the {user-manual}command_line_interface.html#task_name_abbreviation[user manual].
====

[[switch_to_wrapper]]
== Switch to the Gradle Wrapper

Using the Gradle Wrapper has several advantages, one of which is that you don't have to remember which version of Gradle the build was written for. You can learn more about it in the {user-manual}gradle_wrapper.html[user manual].

When you use the wrapper for the first time for a given version of Gradle (regardless of the project), you will download a Gradle distribution of that version. This will happen even if you have that version of Gradle already installed. So expect a short (depending on your internet bandwidth) wait during the next step.

Try running the `copySources` task with the Gradle Wrapper:
...
.On Windows
====
Use `gradlew` in place of `./gradlew` on Windows.
====
...
Gradle comes packaged with a range of core plugins, and you'll find many, many more on http://plugins.gradle.org[the Gradle Plugin Portal]. One of the core plugins — the {user-manual}base_plugin.html[Base Plugin] — provides a `clean` task as well as several other common lifecycle tasks like `assemble`, `check` and `build`.
